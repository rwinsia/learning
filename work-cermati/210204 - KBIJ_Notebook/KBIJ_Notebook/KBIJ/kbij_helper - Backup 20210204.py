    ...
    def get_F01_data(self, year_date, filter=True):
        ...
        sql = """
            ...
            , detail_payment as (
                ...
                from `data-platform-indodana.vayu.indodana_athena_contracts` con
                left join data-platform-indodana.vayu.indodana_athena_user_payments up on up.contractId = con.id and paymentProvider != 'WAIVER' and DATE(paymentProviderReceivedDate) <= date_trunc(DATE({date}), MONTH)
            )
            , count_payment as (
                select orderId
                        , SUM(CASE
            ...
            --         END AS `Frekuensi_Tunggakan`,
                    ...
                    INNER JOIN `athena-179008.vayu_data_mart.indodana_athena_contracts` c ON a.orderId = c.applicationOrderId AND c.dataDate = date_trunc(DATE({date}), MONTH)
