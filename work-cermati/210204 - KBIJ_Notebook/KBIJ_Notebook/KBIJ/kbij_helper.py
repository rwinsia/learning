...
class KBIJ(object):
    ...
    def get_F01_data(self, year_date, filter=True):
        ...
        if filter:
            filter_q = """AND (c.status = 'ACTIVE'
                        OR (c.status IN ('FINISHED', 'FINISHED_RESTRUCTURE') AND date(c.updatedAt) BETWEEN date_trunc(DATE_SUB(DATE({date}), INTERVAL 33 DAY), MONTH) AND date_trunc(DATE({date}), MONTH))
        ...
        sql = """
            with user_virtual_account as (
                        ...
                        from data-platform-indodana.vayu.indodana_athena_virtual_accounts
            ...
            , user_virtual_account_subs AS (
                ...
                FROM (
                    ...
                    FROM `data-platform-indodana.vayu.indodana_athena_virtual_accounts` vc
                    JOIN data-platform-indodana.vayu.indodana_athena_applications a ON vc.userId = a.userId
                    JOIN data-platform-indodana.vayu.indodana_athena_contracts c ON a.orderId = c.applicationOrderId
                    WHERE virtualAccountBank != 'BCA'
            ...
            , detail_payment as (
                select con.athena_orderId as orderId, amount, provider_transaction_id as paymentProviderTransactionId, provider_received_date as paymentProviderReceivedDate, up.created_at as createdAt
                from athena-179008.vayu_data_mart.view_ath_contracts con
                LEFT join athena-179008.vayu_data_mart.view_ath_user_payments up 
                    on up.loan_contract_id = con.loan_contract_id
                    and DATE(provider_received_date) <= date_trunc(DATE({date}), MONTH)
            )
            , count_payment as (
                select orderId
                        , SUM(CASE
            ...
            SELECT '0301' AS `Kode_Jenis_Pelapor`,
                    ...
                    CASE
                        WHEN c.`status` IN ('REJECTED',
                                            'CANCELLED','FINISHED', 'FINISHED_RESTRUCTURE')
                            THEN 0
                        ELSE CASE
            ...
            --         END AS `Tunggakan_Pokok`,
                    ...
                    CASE
                        WHEN c.`status` IN ('FINISHED', 'FINISHED_RESTRUCTURE') THEN 0
            ...
            --         END AS `Frekuensi_Tunggakan`,
                    ...
                    CASE
                        WHEN c.`status` IN ('REJECTED',
                                            'CANCELLED') THEN '01'
                        WHEN c.`status` IN ('FINISHED', 'FINISHED_RESTRUCTURE') THEN '02'
                        ELSE '00'
                    END AS `Kode_Kondisi`,
                    CASE
                        WHEN c.`status` IN ('REJECTED',
                                            'CANCELLED','FINISHED', 'FINISHED_RESTRUCTURE') THEN c.`updatedAt`
                        ELSE NULL
        ...
        SELECT 
            ...
            , approvedLoanTenureInMonth
            , approvedLoanAmount
            , CASE WHEN status='FINISHED_RESTRUCTURED' THEN 'FINISHED_RESTRUCTURE' ELSE status END as status
            , paidOffDate
            , createdAt
            , updatedAt
            , balance
            ...
            , lateFee
            , maxDayPastDue
        FROM athena-179008.vayu_data_mart.indodana_athena_contracts
        WHERE dataDate<'2021-01-23'
        UNION ALL
        select 
            lc.dataDate
            , lc.id as id
            ...
            , cast(lcl.balance_principal as int64) as balance
            , cast(coalesce(lcl.balance_principal,0)+coalesce(lcl.balance_interest,0) as int64) as balanceWithInterest
            , lcl.next_due_date as nextDueDate
            , coalesce(lcl.dpd,0) as dayPastDue
            , lcl.latePaymentCount
            , cast(lcl.balance_latefee as int64) as latefee
        ...
        LEFT JOIN data-platform-indodana.vayu.indodana_athena_contracts c ON lc.id=c.creditLoanContractId
        ...
        LEFT join (
            select 
                ...
                , date_diff(Current_Date(),date(min(case when status in ('UNPAID','PARTIALLY_PAID') then due_date end),"Asia/Jakarta"),day) as dpd
                ...
                , SUM(CASE WHEN type='PRINCIPAL' AND status='PAID' AND date(paid_off_date,"Asia/Jakarta")>date(due_date,"Asia/Jakarta") THEN 1 ELSE 0 END) as latePaymentCount
            from athena-179008.vayu_data_mart.indodana_credit_loan_contract_ledgers 
            where status not in ('CANCELLED')
            group by 1,2
        ) lcl on lcl.loan_contract_id=lc.id AND lcl.dataDate=lc.dataDate
        WHERE ir.type IN ('CASH_LOAN') AND lc.dataDate>='2021-01-23'
) c ON a.orderId = c.applicationOrderId AND c.dataDate = date_trunc(DATE({date}), MONTH)
                    LEFT JOIN data-platform-indodana.vayu.indodana_athena_installment_commitments ic ON c.`id` = ic.`contractId`
    ...
    def get_D01_data(self, year_date, filter=True):
        ...
        sql = """
                ...
                , summary AS (
                    SELECT '0301' AS Kode_Jenis_Pelapor
                        ...
                        ,CAST(TRIM(replace(replace(a.applicantIdNumber,'O','0'),'o','0')) AS STRING) AS Nomor_Identitas
                        ...
                        , regexp_replace(replace(a.currentCompanyName, '|', ''), '[^a-zA-Z0-9\\\\s]', ' ') AS Tempat_Bekerja
                        ,CASE
                        ...
                        ,replace(replace(CAST(regexp_replace(a.currentCompanyAddress, '[^a-zA-Z0-9\\\\s]', ' ') AS STRING), ',', ''), '|', '') AS Alamat_Tempat_Bekerja
        ...
        WHERE ir.type IN ('CASH_LOAN') AND lc.dataDate>='2021-01-23'
                    ...
                    LEFT join `data-platform-indodana.vayu.indodana_athena_ktp_validations` kv on a.applicantIdNumber = kv.nik
                    LEFT JOIN `data-platform-indodana.vayu.indodana_athena_npwp_validations` n ON a.userId = n.userId
                    LEFT JOIN user_virtual_account uv on a.userId  = uv.userId
                    LEFT JOIN user_virtual_account_subs uvs on a.userId  = uvs.userId
                    WHERE  a.orderId NOT IN (SELECT orderId
                                            FROM test_apps)
